#!/usr/bin/env python3
"""
NS2 Bluetooth Monitor (Python) v1.7
With GUI
Joy-Cons are only supported as single controllers and only monitoring.
"""
...
async def notification_callback(sender, data):
    ...
    if pid == PRODUCT_ID_GC:
        ...
        if debug_mode:
            ...
            status_str = f"[GC] Buttons: {btns_display:<30} | Sticks: {axes_display} {trigger_display} | {raw_display}"
        elif verbose_mode:
        ...
        if debug_mode:
            raw_display = print_raw_bytes(data)
            status_str = f"[SW] Buttons: {btns_display:<30} | Axes: {axes_display} | {raw_display}"
        elif verbose_mode:
...
class NS2ControllerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Switch 2 Bluetooth Monitor")
